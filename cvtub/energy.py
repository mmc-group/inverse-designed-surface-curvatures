...
def polykap_deg2(u, params, delta_x, xi, GradHessConv_ZXY) :
    ...
    if orientation and mu_2 != 0 and theta_2 is not None : # add orientation energy
        sca_prod_2 = dz_u * theta_2[0] + dx_u * theta_2[1] + dy_u * theta_2[2]
        integ += (mu_2 * eps * sca_prod_2**2).sum()

    if orientation and mu_3 != 0 and theta_3 is not None : # add orientation energy
        sca_prod_3 = dz_u * theta_3[0] + dx_u * theta_3[1] + dy_u * theta_3[2]
        integ += (mu_3 * eps * sca_prod_3**2).sum()
